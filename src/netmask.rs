...
lazy_static! {
    static ref LOOKUP: HashMap<&'static str, &'static str> = {
        [
            ("/1", "128.0.0.0"), ("/2", "192.0.0.0"), ("/3", "224.0.0.0"),
...
pub fn get_netmask_for_cidr(cidr_suffix: &str) -> Option<String> {
    if let Some(netmask) = LOOKUP.get(cidr_suffix) {
        Some(netmask.clone().to_string())
    } else {
        None
    }
}
